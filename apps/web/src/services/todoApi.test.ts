...
describe("localTodoApi (TodoApiClient mock)", () => {
  ...
  it("createTodo로 TODO를 추가하고, fetchTodos로 조회할 수 있다", async () => {
    const created = await localTodoApi.createTodo("새 할 일");
  ...
  it("updateTodo로 TODO를 수정하면 저장소에도 반영된다", async () => {
    const created = await localTodoApi.createTodo("원래 제목");
    const updated: Todo = { ...created, title: "수정된 제목" };
  ...
  it("deleteTodo로 TODO를 삭제하면 목록에서 사라진다", async () => {
    const created = await localTodoApi.createTodo("삭제될 할 일");
