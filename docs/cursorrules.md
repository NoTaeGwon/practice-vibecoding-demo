...
- 사용자가 별도로 언급하지 않는 한, 이 문서와 `docs/tasks.md`의 흐름에 맞춰 **계획 → 구현 → 커밋**까지 진행한다.
...
- **백엔드**: 백엔드는 **TDD로 구현**한다.
- **체크리스트 갱신**: 커밋 전에 `docs/tasks.md` 에 진행 상황을 업데이트한다.
...
- **문서/설계 규칙**
  ...
  - 새로운 단계 또는 기능을 완료하면 `docs/tasks.md` 에 진행 상황과 커밋 포인트를 반영한다. (루트 규칙 재확인)
...
- 새로운 기능을 추가할 때는 우선 `docs/tasks.md` 에서 해당하는 섹션을 찾는다.
- 다음 순서를 따른다.
